...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, isfloating, isurgent, canfocus, neverfocus, oldstate, isfullscreen, isterminal, noswallow, floatborderpx;
	pid_t pid;
	Client *next;
	Client *snext;
	Client *swallowing;
...
updatestatus(void)
{
  ...
  if (gettextprop(root, XA_WM_NAME, text, sizeof text)) {
      ...
      for (char *rt = text;*rt != *e; rt++) //loops until ends string
              if (*rt == '\0') break; 
              if ((unsigned char)*rt >= ' ')      
                        *(stc++) = *(sts++) = *(stt++) = *rt;
